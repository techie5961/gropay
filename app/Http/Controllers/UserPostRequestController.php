...
class UserPostRequestController extends Controller
{
    ...
    //   withdrawal pin
      $pin=request('pin-1').request('pin-2').request('pin-3').request('pin-4');
      if(!Hash::check($pin,Auth::guard('users')->user()->withdrawal_pin)){
        return response()->json([
            'message' => 'Incorrect withdrawal pin,kindly reset your withdrawal pin if forgotten',
            'status' => 'error'
        ]);
      }
      DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
    ...
    public function UpdateProfile(){
        ...
        if(request()->hasFile('photo')){
            $name=time().'.'.request()->file('photo')->getClientOriginalExtension();
            request()->file('photo')->move(public_path('users'),$name);
            $photo=$name;
        }
        DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
            'name' => request('name'),
    ...
    public function WithdrawalPin(){
        $pin=request('pin-1').request('pin-2').request('pin-3').request('pin-4');
         $confirm_pin=request('confirm-pin-1').request('confirm-pin-2').request('confirm-pin-3').request('confirm-pin-4');
        if(!Hash::check($confirm_pin,Hash::make($pin))){
            return response()->json([
                'message' => 'Withdrawal pin and confirm pin must be the same',
                'status' => 'error'
            ]);
        }
        if(!Hash::check(request('password'),Auth::guard('users')->user()->password)){
            return response()->json([
    ...
    // redeem deposit voucher
    public function RedeemDepositVoucher(){
        ...
        if($voucher->status !== 'active'){
              return response()->json([
                'message' => 'This voucher has already been used,please purchase another voucher',
                'status' => 'error'
            ]);
        }
        DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
...
',
        ...
        return response()->json([
            'message' => 'Voucher has been successfully redeemed,your deposit wallet has been creditted with &#8358;'.number_format($voucher->value,2).'',
            'status' => 'success'
        ]);
    }
